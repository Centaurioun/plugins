...
namespace Nikse.SubtitleEdit.PluginLogic
{
    public class ExportAllFormats : EntryPointBase
    {
        public ExportAllFormats()
            : base("Export to all formats", "Export to all formats (non binary)", 1.1m,
                "Export current subtitle to all available text format.", "file", string.Empty)
        {
        }
            ...
            string file, string videoFile, string rawText)
        {
            ...
            if (string.IsNullOrWhiteSpace(srtText))
            {
                MessageBox.Show(parentForm,
                    "Empty subtitle... make sure you have load a subtitle file before trying to export!",
                    "Empty subtitle", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return string.Empty;
            }
